...
def eagle_ppo_loss(
    ...
    """PPO loss augmented with Eagle3 draft distillation loss.

    Combines the standard PPO policy gradient loss with a soft cross-entropy
    distillation loss that trains the Eagle3 draft model to imitate the policy.

    L_total = L_policy + draft_loss_weight * L_draft

    Args:
        config:            ActorConfig (same as ppo_loss).
        model_output:      Dict containing "log_probs", "logits" (full vocab), etc.
        data:              TensorDict with advantages, old_log_probs, response_mask, etc.
        dp_group:          Data-parallel process group (passed through to ppo_loss).
        draft_logits:      [batch, seq, vocab] logits from the Eagle draft model.
                           If None, falls back to plain ppo_loss with no draft term.
        draft_loss_weight: Multiplier λ for the draft distillation loss.

    Returns:
        (total_loss, metrics) where metrics includes "actor/draft_loss".
    """
    ...
    # Retrieve teacher (policy) logits — model_output must carry "logits"
    teacher_logits = model_output.get("logits", None)
    if teacher_logits is None:
        return policy_loss, metrics

    # Retrieve response mask (already a padded BoolTensor after ppo_loss selected it)
    # Re-extract from raw data before ppo_loss narrowed it to selected fields
    response_mask = data.get("response_mask", None)
    if response_mask is None:
        return policy_loss, metrics
    if hasattr(response_mask, "to_padded_tensor"):
        response_mask = response_mask.to_padded_tensor()
    ...
    draft_loss = compute_eagle_draft_loss_with_alignment(
        ...
        response_mask=response_mask,
        loss_weight=draft_loss_weight,
    )

    total_loss = policy_loss + draft_loss
    metrics["actor/draft_loss"] = Metric(value=draft_loss, aggregation=AggregationType.MEAN)
